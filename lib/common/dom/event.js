...
class EventElement extends inlinejs_element_1.CustomElement {
    ...
    keyIs(event, keys) {
        return (Array.isArray(keys) ? keys : [keys]).some((key) => this.filterKey_(event, k => (k === (0, inlinejs_1.GetGlobal)().GetConfig().MapKeyEvent(key))));
    }
    HandleElementScopeCreated_(_a, postAttributesCallback) {
        ...
        super.HandleElementScopeCreated_(Object.assign({ scope }, rest), () => {
            ...
            if (target && type) {
                const componentId = ((this.context && ((_a = (0, inlinejs_1.InferComponent)(this.context)) === null || _a === void 0 ? void 0 : _a.GetId())) || this.componentId_);
                if ((typeof this.type === 'string' && (this.directive && this.type.includes(':'))) && target instanceof HTMLElement) {
                    const directive = (0, inlinejs_1.CreateDirective)(this.type, (this.textContent || '').trim()); // Example: hx-router:load
                    if (directive && (0, inlinejs_1.DispatchDirective)(componentId, target, directive)) {
                        return;
                    }
                }
                ...
                const evaluateFilter = (typeof this.filter === 'string' && this.filter.trim() ? (0, inlinejs_1.EvaluateLater)({
                    componentId: componentId,
                    contextElement: (this.context || this),
                ...
                const filter = (event) => {
                    ...
                    if (typeof this.filter === 'function') {
                        return (0, inlinejs_1.JournalTry)(() => this.filter(event));
                    }
                ...
                if (this.outside && target instanceof HTMLElement) {
                    subscribe = (e, m) => {
                        (0, inlinejs_1.AddOutsideEventListener)(target, e, handler);
                        m && (0, inlinejs_1.AddOutsideEventListener)(target, m, handler);
                    };
                    unsubscribe = (e, m) => {
                        (0, inlinejs_1.RemoveOutsideEventListener)(target, e, handler);
                        m && (0, inlinejs_1.RemoveOutsideEventListener)(target, m, handler);
                    };
                }
                else {
                    subscribe = (e, m) => {
                ...
                scope.AddUninitCallback(() => (Array.isArray(type) ? type : [type]).forEach((t) => unsubscribe === null || unsubscribe === void 0 ? void 0 : unsubscribe(t, getMappedEvent(t))));
            }
