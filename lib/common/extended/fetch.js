...
class ProgressFetchElement extends inlinejs_element_1.CustomElement {
    ...
    TransformRequestInit_(body) {
        ...
        if ((0, inlinejs_1.IsObject)(body)) {
            const formData = new FormData();
            Object.entries(body).forEach(([key, value]) => formData.append(key, (value instanceof Blob) ? value : String(value)));
            return formData;
        }
        if (body instanceof ReadableStream) {
            (0, inlinejs_1.JournalWarn)('ProgressFetchElement does not support ReadableStream bodies.', 'hx-progress-fetch::TransformRequestInit_', this);
            return null;
        }
        return body;
    }
